...
impl IscsiPdu {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        // Bytes 8-15: LUN, or ISID+TSIH for Login PDUs (RFC 3720 §§10.12 / 10.13).
        let write_session_ids = matches!(
            self.opcode,
            opcode::SCSI_COMMAND
...
mod tests {
    ...
    #[test]
    fn test_login_response_serializes_isid() {
        let isid = [0x08, 0xae, 0xdd, 0x68, 0x00, 0x65];
        let pdu = IscsiPdu::login_response(
            isid,
            0,     // tsih (still 0 during security)
            1, 1, 2,
            0, 0,
            0, 1,  // CSG=0 NSG=1
            true,
            1,
            b"AuthMethod=None\0".to_vec(),
        );
        let bytes = pdu.to_bytes();
        assert_eq!(bytes[0], opcode::LOGIN_RESPONSE);
        assert_eq!(bytes[1] & flags::TRANSIT, flags::TRANSIT);
        assert_eq!(&bytes[8..14], &isid);
        assert_eq!(&bytes[14..16], &[0, 0]); // TSIH
    }
