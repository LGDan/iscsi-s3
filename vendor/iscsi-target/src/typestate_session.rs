        ...
        match key {
            ...
            "MaxRecvDataSegmentLength" => {
                if let Ok(v) = value.parse::<u32>() {
                    // Declarative: initiator announces what *it* can receive (our xmit limit).
            ...
            "MaxConnections" => {
                // RFC 3720 12.2: minimum result function; default 1.
                if let Ok(v) = value.parse::<u32>() {
                    self.params.max_connections = v.min(self.params.max_connections).max(1);
                }
            }
            "DataPDUInOrder" => {
            ...
            _ => {
                log::info!("Unknown login key (will answer NotUnderstood): {}={}", key, value);
                return false;
            }
        }
        true
...
impl TypestateSession<Free> {
    ...
    pub fn process_login(
        ...
        for (key, value) in &login.parameters {
            let _ = self.data.apply_initiator_param(key, value);
        }
        ...
        let param_summary: Vec<String> = login
            ...
            .collect();
        log::info!(
            "Login PDU: CSG={} NSG={} transit={} ISID={:02x?} CID={} CmdSN={} params=[{}]",
            login.csg,
            login.nsg,
            login.transit,
            login.isid,
...
impl TypestateSession<SecurityNegotiation> {
    ...
    pub fn process_login(
        ...
        let param_summary: Vec<String> = login
            ...
            .collect();
        log::info!(
            "Login PDU (SecurityNegotiation): CSG={} NSG={} transit={} params=[{}]",
            login.csg,
            login.nsg,
            login.transit,
            param_summary.join(", ")
    ...
    fn continue_login(
        ...
        let (auth_complete, mut auth_params) = match self.handle_chap_auth(&login.parameters) {
            Ok(result) => result,
            Err(e) => {
        ...
        // First security response should include TargetPortalGroupTag when TargetName was given.
        if !auth_params.iter().any(|(k, _)| k == "TargetPortalGroupTag")
            && (!self.data.params.target_name.is_empty()
                || login.parameters.iter().any(|(k, _)| k == "TargetName"))
        {
            // Prepend so AuthMethod stays last-ish in common traces; order is not significant.
            auth_params.insert(0, ("TargetPortalGroupTag".to_string(), "1".to_string()));
        }

        // Auth still in progress (e.g. CHAP challenge): reply without transit and stay.
        if !auth_complete {
            ...
            log::info!(
                "Login security reply (auth in progress): T=0 CSG=0 NSG=0 params=[{}]",
                auth_params
                    .iter()
                    .map(|(k, v)| {
                        if k.starts_with("CHAP_") {
        ...
        // Auth complete. If the initiator requested transit out of security, honour it and
        // include any final security keys (e.g. AuthMethod=None) in the same response.
        // Previously AuthMethod=None took an early "stay in security" path with T=0, which
        // open-iscsi tolerated with a follow-up PDU but Intel NUC firmware aborts on.
        ...
        if transit {
            match (login.csg, login.nsg) {
                (0, 1) => {
                    ...
                    let response_data = serialize_text_parameters(&auth_params);
                    log::info!(
                        "Login security complete: transit CSG=0->NSG=1 initiator={} params=[{}]",
                ...
                (0, 3) => {
                    ...
                    let mut response_params = auth_params;
                    if self.data.session_type != SessionType::Discovery {
                        response_params.extend(self.data.generate_response_params());
                    }
                    let response_data = serialize_text_parameters(&response_params);
                    log::info!(
                        "Login security complete: transit CSG=0->NSG=3 (FullFeature) initiator={}",
                        self.data.params.initiator_name
                    );
                ...
                _ => {
                    ...
                    self.data.stat_sn = self.data.stat_sn.wrapping_add(1);
                    log::warn!(
                        "Login transit requested with unsupported CSG={} NSG={}",
                        login.csg, login.nsg
                    );
                    let response = IscsiPdu::login_response(
                        self.data.isid, self.data.tsih, self.data.stat_sn,
                        self.data.exp_cmd_sn, self.data.max_cmd_sn,
                        0, 0, 0, 1, false, itt, serialize_text_parameters(&auth_params),
                    );
                    Ok((AnySession::SecurityNegotiation(self), vec![response]))
                }
            }
        } else {
            // No transit - acknowledge security keys and stay in SecurityNegotiation
            self.data.stat_sn = self.data.stat_sn.wrapping_add(1);
            log::info!(
                "Login security reply (no transit): T=0 CSG=0 NSG=0 params=[{}]",
                auth_params
                    .iter()
                    .map(|(k, v)| format!("{k}={v}"))
                    .collect::<Vec<_>>()
            ...
            let response = IscsiPdu::login_response(
                self.data.isid, self.data.tsih, self.data.stat_sn,
                self.data.exp_cmd_sn, self.data.max_cmd_sn,
                0, 0, 0, 0, false, itt, serialize_text_parameters(&auth_params),
            );
            Ok((AnySession::SecurityNegotiation(self), vec![response]))
        }
    }
...
impl TypestateSession<LoginOperationalNegotiation> {
    ...
    pub fn process_login(
        ...
        let param_summary: Vec<String> = login
            ...
            .collect();
        log::info!(
            "Login PDU (OperationalNegotiation): CSG={} NSG={} transit={} initiator={} params=[{}]",
            login.csg,
            login.nsg,
            login.transit,
            self.data.params.initiator_name,
        ...
        if login.transit && login.nsg == 3 {
            ...
            let response_summary: Vec<String> = response_params
                ...
                .collect();
            log::info!(
                "Login operational complete: transit to FullFeature initiator={} response=[{}]",
                self.data.params.initiator_name,
            ...
            // No transit - report our current stage (still answer any unknown keys)
            self.data.stat_sn = self.data.stat_sn.wrapping_add(1);
            ...
            let response = IscsiPdu::login_response(
                self.data.isid, self.data.tsih, self.data.stat_sn,
                self.data.exp_cmd_sn, self.data.max_cmd_sn,
                0, 0, 1, 1, false, itt, serialize_text_parameters(&response_params),
            );
